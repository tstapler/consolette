...
impl CodeCompressor {
    ...
    /// # Panics
    ///
    /// Never panics: every byte range in `comment_ranges` comes from
    /// tree-sitter node boundaries within `code`, so all slices stay in
    /// bounds.
    #[must_use]
...
/// # Panics
///
/// Never panics: every slice boundary below is derived from a preceding
/// `find` call's returned offset (plus a fixed, already-matched marker
/// length), so all indices stay in bounds and on `char` boundaries.
#[must_use]
pub fn compress_fenced_blocks(text: &str) -> Option<String> {
    ...
    while let Some(fence_start) = text[cursor..].find("```") {
        ...
        } else if let Some(stripped) = compressor.compress(body, lang) {
