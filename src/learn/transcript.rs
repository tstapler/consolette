...
/// # Errors
///
/// Returns an error if the `HOME` environment variable is not set or if the
/// glob pattern itself is malformed.
...
#[must_use]
pub fn parse_file(path: &Path) -> Vec<TranscriptEntry> {
    ...
    for (line_num, line) in content.lines().enumerate() {
        ...
        let role = message.role.as_deref().unwrap_or(entry_role).to_string();

        let content = extract_plain_text_content(message.content.as_ref());
        if content.is_empty() {
            continue;
        }
