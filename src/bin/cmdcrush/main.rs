...
//!
//! Reuses `consolette::compression::TextCompressor`, the same native
...
#[allow(clippy::struct_excessive_bools)] // independent clap flags, not a state-machine struct
struct Cli {
...
#[allow(clippy::cast_precision_loss)] // byte counts as f64 for a percentage display; not exact-value-critical
...
#[allow(clippy::too_many_lines)] // parse-then-print report function; splitting would fragment one cohesive scan
#[allow(clippy::cast_precision_loss)] // token counts as f64 for a percentage display; not exact-value-critical
...
#[allow(clippy::too_many_lines)] // top-level orchestration, already sectioned by early returns
fn main() {
    let cli = Cli::parse();
    ...
    // `Stdio::piped()` was set on both handles above, so `take()` is
    // structurally guaranteed `Some` here; a scoped allow is clearer than
    // threading an unreachable error case through `main`'s control flow.
    ...
    let (stdout_text, stderr_text) = match (stdout_text, stderr_text) {
        (Some(o), Some(e)) => (o, e),
        (stdout_opt, stderr_opt) => {
            if stdout_opt.is_none() && has_stdout {
                eprintln!("cmdcrush: stdout is not valid UTF-8 — passing through uncompressed");
            }
            if stderr_opt.is_none() && has_stderr {
