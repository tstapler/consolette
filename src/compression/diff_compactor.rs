...
/// # Panics
///
/// Never panics: the edge slices below only execute when
/// `run.len() > CONTEXT_COLLAPSE_THRESHOLD`, which is always greater than
/// `2 * CONTEXT_KEEP_EDGE`, so both slices stay in bounds.
#[must_use]
...
mod tests {
    ...
    fn sample_diff(context_lines: usize) -> String {
        ...
        for n in 0..context_lines {
            let _ = writeln!(s, " context line {n}");
        }
        s.push_str("-old line\n");
        s.push_str("+new line\n");
        s
