...
pub fn format_rewind_marker(items_before: usize, items_after: usize, hash_id: &str) -> String {
    format!("[{items_before} items compressed to {items_after}. Retrieve: hash={hash_id}]")
}
...
impl RewindStore {
    ...
    /// # Panics
    ///
    /// Never panics in practice: `Sha256::digest` always returns a
    /// fixed-length 32-byte output, so indexing the first 4 bytes is always
    /// in bounds.
