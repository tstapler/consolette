...
impl ToF64 for u64 {
    #[allow(clippy::cast_precision_loss)] // metric values; loss above 2^52 is not a concern here
    fn to_f64(self) -> f64 {
        self as f64
    }
}
...
impl PushMetricExporter for SqliteMetricsExporter {
    // `PushMetricExporter::export` requires an `async fn` signature; this
...
pub fn default_db_path() -> PathBuf {
    ...
    let base = std::env::var("HOME").map_or_else(|_| std::env::temp_dir(), PathBuf::from);
