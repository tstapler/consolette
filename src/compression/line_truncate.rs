...
/// # Panics
///
/// Never panics: the early return above guarantees `head` and `tail` are
/// both within `lines.len()` whenever the slicing below executes.
#[must_use]
pub fn truncate_lines(text: &str, head: usize, tail: usize) -> String {
    ...
    out.extend(lines[..head].iter().map(ToString::to_string));
    out.push(format!("[... {omitted} lines omitted ...]"));
    out.extend(lines[lines.len() - tail..].iter().map(ToString::to_string));
    out.join("\n")
}
