...
static ANSI_ESCAPE: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"\x1b\[[0-9;]*[mK]").expect("ANSI_ESCAPE regex"));

/// Matches an ISO 8601 timestamp embedded in a log line.
/// Captures the prefix before, the timestamp itself, and the suffix after.
// Same rationale as `ANSI_ESCAPE`: the pattern is a fixed string constant.
#[allow(clippy::expect_used)]
...
fn dedup_consecutive_lines(text: &str) -> String {
    ...
    while i < lines.len() {
        ...
        if count >= 3 {
            ...
            out.push(format!("[{remaining} more identical lines]"));
        } else {
            out.extend(std::iter::repeat_n(current.to_string(), count));
        }
...
fn dedup_log_timestamps(text: &str) -> String {
    ...
    while i < annotated.len() {
        ...
        if count >= 3 {
            out.push(line.to_string());
            let remaining = count - 1;
            out.push(format!("[{remaining} more identical log lines]"));
        } else {
            for j in 0..count {
...
mod tests {
    ...
    fn full_pipeline_shrinks_progress_bar_output() {
        ...
        let bar: String = (0..=100).fold(String::new(), |mut acc, p| {
            let _ = write!(acc, "\rDownloading... {p}%");
            acc
        });
