...
3. **MCP `session_id` transport binding.** Task 5.1.0's spike (2026-08-13, see ADR-009) confirmed Claude Code spawns one MCP subprocess per session, but found no confirmed way to read the session ID from that process's environment at spawn time — so `read_omitted_content`'s `session_id` ships as a client-supplied argument in v1, not derived from an independently-verified per-connection identity. Revisit if consolette's MCP transport later confirms a reliable spawn-time session-ID signal.
4. **Empirical boundary-row verification outcome.** ADR-011 was updated 2026-08-14 with empirical evidence from real, native `compact_boundary` rows on this machine, confirming the documented `type: "system"`/`subtype: "compact_boundary"` shape (not magic-compact's `type: "user"`/`isMeta: true` shape) — see ADR-011's "Empirical Verification" section. Task 4.1.1e itself (running `consolette compact-session` against a real transcript and confirming `claude --resume <new-session-id>` actually works) remains outstanding — it requires a live, interactive Claude Code CLI session and is not automatable in CI.
