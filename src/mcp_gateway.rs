...
/// Cached tool list plus the instant it was populated, guarded by an `RwLock`.
type ToolCacheEntry = Option<(Vec<Tool>, Instant)>;

#[derive(Clone)]
struct ToolCache {
    inner: Arc<RwLock<ToolCacheEntry>>,
    ttl: Duration,
}
...
pub async fn build_mcp_router() -> Option<Router> {
    ...
    for (server_name, server_cfg) in &config.servers {
        let Some(url) = server_cfg.upstream_url.clone() else {
