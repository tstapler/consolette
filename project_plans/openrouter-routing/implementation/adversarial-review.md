...
- [x] **RESOLVED — mixed `openrouter_scored` + paid-upstream route now has a config-time guard.** Task 4.3.1g (plan.md) extends Story 4.3.1's validation pass: a route whose `strategy == Strategy::OpenrouterScored` that also lists any non-`openrouter`-kind upstream now fails `from_config` with an error naming the route and the offending paid upstream, with a concrete Given-When-Then (Story 4.3.1's Acceptance Criteria) and a dedicated unit test. Story 4.2.4's own "mixed route" test remains a pure-function `expand_candidates` robustness test (that config shape now can never reach `Router::dispatch` in practice, since Task 4.3.1g rejects it at load time).
  - **Correction:** this item was marked resolved prematurely — the original Epic 4.3 implementation (commit `991a3bf`) never actually added the reverse-direction check described above; `validate_openrouter_strategy_pairing` in `src/routing/router.rs` only checked `is_openrouter && strategy != OpenrouterScored`, never the reverse, so an `openrouter_scored` route mixing in a paid upstream loaded successfully and `OpenrouterScoringStrategy::select()` had no filter preventing it from scoring/dispatching to that paid candidate. Genuinely resolved in the Phase 5 spec-compliance repair loop (commit titled `fix(openrouter): reverse mixed-upstream validation guard + defensive index filter in select() (spec-compliance repair, Task 4.3.1g)`, see this branch's `git log`), which adds both the reverse-direction check in `validate_openrouter_strategy_pairing` and a defense-in-depth filter in `OpenrouterScoringStrategy::select()` (`src/routing/openrouter_scoring.rs`), plus `from_config_should_reject_openrouter_scored_route_mixing_paid_upstream` and `select_should_never_choose_a_candidate_outside_the_openrouter_index`.
