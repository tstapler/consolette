...
pub async fn handler_memory_list(
    ...
    let items: Vec<MemoryListItem> = all
        ...
        .map(|(key, entry)| {
            ...
            // `ttl_secs` values are always small (seconds, well under i64::MAX);
            // saturate rather than wrap in the pathological case.
            let ttl_secs_i64 = i64::try_from(entry.ttl_secs).unwrap_or(i64::MAX);
            let ttl_remaining_secs = ttl_secs_i64 - age_secs;
            MemoryListItem {
                key,
