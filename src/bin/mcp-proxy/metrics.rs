//! MCP filter metrics — in-process counters + shared-file IPC to consolette.
//!
//! mcp-proxy runs as a stdio subprocess; the only cross-process communication
//! path to the consolette dashboard is a JSON file at a well-known path.
...
/// Default path: `~/.cache/consolette/mcp-filter-metrics.json`.
pub fn default_metrics_path() -> PathBuf {
    let home = std::env::var("HOME").unwrap_or_default();
    PathBuf::from(home).join(".cache/consolette/mcp-filter-metrics.json")
}
...
/// the consolette metrics handler.
