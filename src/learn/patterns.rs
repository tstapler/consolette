...
// Pattern is a compile-time constant validated by the tests below; a
// build-time failure here would be a programmer error, not a runtime one.
...
/// Generic error/failure signal words used by the Tier 3 structural detector.
// Pattern is a compile-time constant validated by the tests below; a
// build-time failure here would be a programmer error, not a runtime one.
#[allow(clippy::expect_used)]
static ERROR_SIGNAL: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"(?i)\b(error|fail|wrong|didn't work|not working|broken)\b")
...
fn find_error_recovery_loops(entries: &[TranscriptEntry]) -> Vec<CorrectionPattern> {
    ...
    for (i, entry) in entries.iter().enumerate() {
        ...
        while let Some(uid) = current_uuid {
            let Some(&idx) = uuid_to_idx.get(uid) else {
                break;
            };
            ...
            if ancestor.role == "assistant" {
                ...
                if let Some(grandparent_uid) = ancestor.parent_uuid.as_deref() {
                    if let Some(&gp_idx) = uuid_to_idx.get(grandparent_uid) {
                        ...
                        {
                            loop_count += 1;
                            first_correction.clone_from(&gp.content);
