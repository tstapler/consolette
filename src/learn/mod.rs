...
//! # Safety constraint
//!
//! `learn_apply` writes ONLY to `/tmp/consolette-learn-staging.md`.
...
/// # Errors
///
/// Returns an error if the blocking transcript-parsing task panics, is
/// cancelled, or if parsing the transcripts themselves fails.
...
///
/// The staging file is `/tmp/consolette-learn-staging.md`.
...
// Stays `async` for API consistency with `learn_preview` and its axum handler
// signature, even though the current implementation has no `.await` points.
#[allow(clippy::unused_async)]
pub async fn learn_apply(patterns: &[CorrectionPattern]) -> Result<LearnApplyResponse> {
    const STAGING_PATH: &str = "/tmp/consolette-learn-staging.md";
...
/// # Errors
///
/// Returns an error if the staging file cannot be created or written to.
...
mod tests {
    ...
    fn test_write_staging_file_creates_correct_sections() {
        ...
        assert!(content.contains("# consolette learn — Staging"));
    ...
    fn test_staging_path_is_tmp() {
        ...
        assert!(
            "/tmp/consolette-learn-staging.md".starts_with("/tmp/"),
