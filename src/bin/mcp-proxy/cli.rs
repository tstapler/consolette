...
pub fn run_init(config_path: &std::path::Path, yes: bool) -> anyhow::Result<()> {
...
pub fn run_analyze(_config: &crate::config::McpProxyConfig, sessions: usize, _yes: bool) {
    println!("Analyzing {sessions} recent sessions for tool usage patterns...");
...
pub async fn run_sync(
    ...
    for (server_name, server_cfg) in &config.servers {
        let _url = if let Some(u) = &server_cfg.upstream_url {
            u.clone()
        } else {
