...
impl UpstreamClient {
            ...
            UpstreamTransportKind::Sse if is_atlassian_url(url) => {
                anyhow::bail!(
                    "SSE transport is not supported for Atlassian-hosted servers (deprecated June 30, 2026). \
                     Update mcp-proxy.toml: set transport = 'streamable-http' for server '{server_name}'"
                );
            }
            UpstreamTransportKind::Stdio => {
                anyhow::bail!(
                    "server '{server_name}' uses stdio transport but has upstream_url instead of upstream_command"
                );
            }
            _ => {}
        }
    ...
    pub async fn list_tools(&self) -> anyhow::Result<Vec<Tool>> {
        ...
        let result = tokio::time::timeout(self.timeout, peer.list_all_tools())
            .await
            .with_context(|| {
                let (server_name, timeout_ms) = (&self.server_name, self.timeout.as_millis());
                format!("upstream server '{server_name}' timed out after {timeout_ms}ms")
            })?
            .with_context(|| {
                let server_name = &self.server_name;
                format!("list_tools from upstream server '{server_name}'")
            })?;
        Ok(result)
    }
    ...
    pub async fn call_tool(&self, params: CallToolRequestParams) -> anyhow::Result<CallToolResult> {
        ...
        let result = tokio::time::timeout(self.timeout, peer.call_tool(params))
            ...
            .with_context(|| {
                let (server_name, timeout_ms) = (&self.server_name, self.timeout.as_millis());
                format!("upstream server '{server_name}' timed out calling tool '{tool_name}' after {timeout_ms}ms")
            })?
            .with_context(|| {
                let server_name = &self.server_name;
                format!("call_tool '{tool_name}' from upstream server '{server_name}'")
            })?;
        Ok(result)
    }
}
...
/// Preserved from the legacy upstream client API; not yet called by
/// `server.rs`'s current dispatch path.
#[allow(dead_code)]
pub fn upstream_timeout_error(server_name: &str, timeout_ms: u64) -> rmcp::ErrorData {
    rmcp::ErrorData::internal_error(
        format!("upstream server unreachable: {server_name} (timeout after {timeout_ms}ms)"),
...
pub fn upstream_transport_error(server_name: &str, err: &anyhow::Error) -> CallToolResult {
    CallToolResult::error(vec![ContentBlock::text(format!(
        "Upstream server '{server_name}' error: {err}"
    ))])
}
