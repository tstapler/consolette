...
impl SchemaCompressor {
    ...
    pub fn compress(&self, tools: Vec<Tool>) -> Vec<Tool> {
        match self.level {
            ...
            CompressionLevel::Light => tools.into_iter().map(Self::compress_light).collect(),
            CompressionLevel::Aggressive => {
                tools.into_iter().map(Self::compress_aggressive).collect()
            }
        }
    }
    ...
    fn compress_light(mut tool: Tool) -> Tool {
    ...
    fn compress_aggressive(mut tool: Tool) -> Tool {
        tool.description = tool
            .description
            .as_deref()
            .map(|d| first_sentence(d, 200))
...
fn strip_array(arr: &mut [Value], defs: &HashMap<String, Value>, depth: usize) {
...
mod tests {
    ...
    #[allow(clippy::needless_pass_by_value)] // test helper; call sites pass json!(...) inline
