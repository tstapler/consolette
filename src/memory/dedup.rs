...
impl DedupState {
    /// # Panics
    ///
    /// Never panics in practice: the hardcoded `lshdedup-core` config below is
    /// known valid at compile time, so `LshIndex::new` can never return `Err`.
    #[must_use]
    pub fn new() -> Self {
        ...
        // The config above is a hardcoded constant known to be valid; `LshIndex::new`
        // only errors on invalid config, which cannot happen here.
...
pub async fn find_duplicate(
    ...
    let Ok(text) = std::str::from_utf8(new_value) else {
        return None;
    };
    ...
    .await
    .ok()
    .flatten(); // JoinError → treat as no match
